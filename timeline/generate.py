...
from timeline.file_processors.google_takeout import (
...
def get_timeline_files_in_paths(
    paths, includerules, ignorerules
) -> Iterable[TimelineFile]:
...
def process_timeline_files(
    ...
    if can_process_videos():
        ...
        logging.warning("ffmpeg is not installed. Videos will not be processed.")
    ...
    for file in db.get_unprocessed_timeline_files(cursor):
        ...
        entry_generator = chain(
            *[
                process_function(file, metadata_root)
...
def generate_financial_report(cursor, output_path: Path):
    ...
    balances_by_date = {}
    first_balance_date = (
        None  # The first recorded account balance. There is no history before that.
    )
    ...
    current_date = first_balance_date or datetime.now().date()
    ...
    for current_date_str in balances_by_date:
        balances_by_date[current_date_str]["total"] = {
            "amount": sum(
                [b["amount"] for b in balances_by_date[current_date_str].values()]
            ),
            "date": max(b["date"] for b in balances_by_date[current_date_str].values()),
            "transactionAmount": transaction_amount_by_day.get(
                current_date_str, Decimal(0)
            ),
        }

    with output_path.open("w") as json_file:
        json.dump(balances_by_date, json_file, cls=DecimalEncoder)
...
def generate(
    ...
    process_timeline_files(
        cursor, input_paths, includerules, ignorerules, metadata_root
    )
    connection.commit()
