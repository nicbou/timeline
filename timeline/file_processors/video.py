...
video_geolocation_regex = re.compile(
    r"^(?P<lat>[+-]\d+\.\d+)(?P<lng>[+-]\d+\.\d+)(?P<alt>[+-]\d+\.\d+)?/.*$"
)
...
def make_preview(
    ...
    try:
        ...
        preview_intervals = [
            ...
            for sample_start in [
                int(i / sample_count * video_duration) for i in range(0, sample_count)
            ]
        ]
        ...
        ffmpeg_filter += "".join(f"[v{i}]" for i in range(0, len(preview_intervals)))
        ...
        ffmpeg_filter += f"[allclips]scale=ceil(iw*min(1\\,min({max_width}/iw\\,{max_height}/ih))/2)*2:-2[out]"
...
def process_video(file: TimelineFile, metadata_root: Path):
    ...
    if ffprobe_data["format"].get("tags", {}).get("location"):
        lat, lng, alt = parse_video_geolocation(
            ffprobe_data["format"]["tags"]["location"]
        )
        entry_data["location"] = {
            "latitude": float(lat),
            "longitude": float(lng),
    ...
    if ffprobe_data["format"].get("tags", {}).get("creation_time"):
        ...
        date_start = datetime.strptime(
            ffprobe_data["format"]["tags"]["creation_time"], "%Y-%m-%dT%H:%M:%S.%fZ"
    ...
    if not output_path.exists():
        try:
            make_preview(
                file.file_path, output_path, entry_data["media"]["duration"], 800, 600
            )
        except:
