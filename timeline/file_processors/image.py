...
image_extensions = {
    ext
    for ext, f in Image.registered_extensions().items()
    if f in Image.OPEN and ext != ".psd"
}
...
def get_image_metadata(image_path: Path) -> dict:
    ...
    with Image.open(
        image_path
    ) as image:  # https://github.com/python-pillow/Pillow/issues/4007
        image.verify()

    with Image.open(image_path) as image:
        metadata["media"]["width"], metadata["media"]["height"] = image.size
        assert metadata["media"]["width"] > 0 and metadata["media"]["height"] > 0, (
    ...
    if "Orientation" in exif:
        orientation_map = {
            ...
            6: 270,
            8: 90,
        }  # 0 is not an official value, but it happens
        try:
            metadata["media"]["orientation"] = orientation_map[exif["Orientation"]]
        except KeyError:
            logger.warning(
                f"{image_path} has unexpected EXIF orientation: {exif['Orientation']}"
            )

        # Set the correct width/height according to EXIF orientation
        if (
            metadata["media"]["orientation"] == 90
            or metadata["media"]["orientation"] == 270
        ):
    ...
    if "Make" in exif or "Model" in exif:
        metadata["media"]["camera"] = (
            f"{exif.get('Make', '')} {exif.get('Model', '')}".replace(
                "\x00", ""
            ).strip()
        )
            ...
            try:
                metadata["location"] = {
                    "latitude": parse_exif_coordinate(
                        ...
                        exif["GPSInfo"].get("GPSLatitudeRef"),
                    ),
                    "longitude": parse_exif_coordinate(
                        exif["GPSInfo"]["GPSLongitude"],
                        exif["GPSInfo"].get("GPSLongitudeRef"),
                    ),
                }
            except ValueError:
                logger.warning(
    ...
    # Date
    if "GPSDateStamp" in exif.get("GPSInfo", {}) and "GPSTimeStamp" in exif.get(
        "GPSInfo", {}
    ):
        ...
        try:
            gps_time = ":".join(
                f"{int(timefragment):02}" for timefragment in gps_time_fragments
            )
            gps_datetime = f"{gps_date} {gps_time}"
            metadata["media"]["creation_date"] = parse_exif_date(gps_datetime).replace(
                tzinfo=timezone.utc
            )
        except ValueError:
            logger.warning(
                f"Could not parse EXIF GPS date '{gps_datetime}' - {image_path}"
            )
...
def make_thumbnail(
    image_path: Path, output_path: Path, max_width: int, max_height: int
):
    with Image.open(image_path) as image:
